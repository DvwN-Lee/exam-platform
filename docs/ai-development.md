...
Claude Code를 AI Pair Programmer로 활용하되, Human-in-the-Loop 원칙에 따라 AI 생성 출력은 반드시 개발자 검토·수정을 거쳐 커밋한다. 개발자가 최종 결정권을 갖는 협업 구조이며, 아키텍처 결정과 코드 품질 판단은 인간이 주도한다.

---

## 2. 도구 구성

### 2.1 Claude Code 설정 체계 (AI Agent Guardrails)

`settings.local.json` 기반 Permission Policy로 9개 도메인에서 AI Agent의 실행 권한을 세밀하게 제어한다.
...
### 2.2 MCP 서버 연동 (Tool Use)

5개 MCP(Model Context Protocol) 서버를 Tool Use 인터페이스로 연동하여 AI Agent가 외부 도구를 직접 호출하는 개발 워크플로우를 통합한다.
...
### 2.3 Memory 시스템 (Long-term Context Management)

세션 간 컨텍스트를 유지하기 위해 파일 기반 Long-term Context Management를 운영한다.
...
단일 AI 에이전트가 검토할 때 발생하는 편향을 방지하기 위해, 역할이 다른 에이전트를 병렬 투입하는 Multi-Agent Orchestration을 적용한다. 각 에이전트는 독립적으로 분석을 수행하고, Agent Consensus Protocol(VETO 메커니즘)을 통해 합의에 도달한 후에만 구현을 진행한다.

### 4.1 에이전트 역할 분리

분석 목적에 따라 역할이 다른 에이전트를 구성하여 단일 관점의 편향을 방지한다. 핵심 원칙:

- **독립 분석**: 각 에이전트는 서로의 결과를 모른 채 독립적으로 의견을 제출한다
- **역할 특화**: 에이전트별로 검증 관점(사실 확인, 품질 평가, 구조 분석 등)을 명확히 분리한다
- **교차 검증**: Phase 2에서 결과를 대조하여 불일치 항목을 식별한다

구체적인 역할 구성은 적용 사례(§4.3, §4.4)에서 기술한다.

### 4.2 2단계 프로세스: Agent Consensus Protocol
...
### 4.3 적용 사례 1: 문서 수치 검증

이 문서(`ai-development.md`) 초기 작성 시점(커밋 288건, Phase 6 완료 시)에 3개 에이전트를 투입했다. 아래 수치는 해당 시점의 실측값이다.
...
초기 내부 계획 문서에 기재된 수치 중 다수가 실제와 다름이 드러났다.
...
### 4.4 적용 사례 2: 키워드 정합성 검증

이 문서의 기술 키워드가 실제 구현 내용과 일치하는지 agent teams scrum으로 검증했다. 3개 에이전트를 투입하여 키워드 조사, 문서 분석, 종합 판단을 병렬로 수행했다.

| 에이전트 | 역할 | 분석 범위 |
|----------|------|-----------|
| `keyword-researcher` | 업계 키워드 조사 | AI Engineer 분야 기술 용어 분류 (Tier 1: 구현 증거 있음, Tier 2: 경험 수준, Tier 3: 미구현) |
| `doc-analyst` | 문서 키워드 분석 | 기존 문서의 기술 용어와 실제 구현 내용 간 정합성 검증 |
| `strategist` | 종합 판단 | 양쪽 결과 크로스체크 및 Tier 분류 최종 확정 |

크로스체크 과정에서 RAG, LangChain 등 Tier 3(미구현) 키워드 추가가 제안되었으나, 실제 구현하지 않은 기술을 문서에 포함하는 것은 Agent Consensus Protocol의 VETO 원칙에 위배된다는 판단으로 기각했다. 최종 합의는 Tier 1(구현 증거 있음) 키워드만 사용하고, 기존 기술 용어를 업계 표준 표현으로 정리하는 것이었다.

### 4.5 구현 단계: Subagent-Driven Development
...
## 6. Prompt Engineering 인사이트

### 6.1 Permission Policy 설계
...
- **Human-in-the-Loop 검증**: 모든 AI 출력을 개발자가 리뷰하는 원칙이 품질 유지에 핵심이다
...
- Co-Authored-By 태그는 Claude Code가 `git commit`을 직접 실행한 13건에 적용됨 — 대부분의 커밋은 개발자가 검토·수정 후 직접 커밋하는 구조

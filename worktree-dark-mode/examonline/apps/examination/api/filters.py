...
class ExaminationFilter(django_filters.FilterSet):
    ...
    exam_state = django_filters.ChoiceFilter(
        choices=ExaminationInfo._meta.get_field("exam_state").choices
    )
    exam_type = django_filters.ChoiceFilter(
        choices=ExaminationInfo._meta.get_field("exam_type").choices
    )

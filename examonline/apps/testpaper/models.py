...
class TestPaperInfo(models.Model):
    ...
    subject = models.ForeignKey(
        SubjectInfo, on_delete=models.PROTECT, verbose_name="시험지 과목", default=""
    )
    tp_degree = models.CharField(
        choices=(("jd", "쉬움"), ("zd", "중간"), ("kn", "어려움")),
        max_length=2,
        verbose_name="시험지 난이도",
        default="jd",
    )
    ...
    create_user = models.ForeignKey(
        UserProfile, on_delete=models.SET_NULL, null=True, verbose_name="작성자"
    )
...
class TestPaperTestQ(models.Model):
    ...
    test_question = models.ForeignKey(
        TestQuestionInfo, on_delete=models.CASCADE, verbose_name="시험 문제"
    )
...
class TestScores(models.Model):
    ...
    test_paper = models.ForeignKey(
        TestPaperInfo, on_delete=models.SET_NULL, null=True, verbose_name="시험지 정보"
    )

...
class ScoresViewSet(viewsets.ViewSet):
    ...
    def exam_scores(self, request, exam_id=None):
        ...
        if request.user.user_type != "teacher":
            return Response(
                {"detail": "교사만 접근할 수 있습니다."}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        # 시험 등록된 모든 학생의 성적 조회
        scores = (
            TestScores.objects.filter(exam=exam)
            .select_related("user", "test_paper")
            .order_by("-is_submitted", "-test_score")
        )

        serializer = ExamScoreListSerializer(scores, many=True)
        return Response(
            {"exam_id": exam.id, "exam_name": exam.name, "scores": serializer.data},
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="exam/(?P<exam_id>[^/.]+)/student/(?P<student_id>[^/.]+)",
    )
    def student_score_detail(self, request, exam_id=None, student_id=None):
        ...
        # 학생 성적 조회
        try:
            score = TestScores.objects.select_related(
                "exam", "exam__subject", "test_paper", "user"
            ).get(exam=exam, user_id=student_id)
        except TestScores.DoesNotExist:
            return Response(
    ...
    def manual_grade(self, request, pk=None):
        ...
        return Response(
            {"detail": "채점이 완료되었습니다.", "new_total_score": score.test_score},
            status=status.HTTP_200_OK,
        )

...
def teacher_user(db):
    return UserProfile.objects.create_user(
        username="teacher_scores",
        password="testpass123",
        user_type="teacher",
        nick_name="Scores Teacher",
    )
...
def another_teacher(db):
    return UserProfile.objects.create_user(
        username="teacher2_scores",
        password="testpass123",
        user_type="teacher",
        nick_name="Another Teacher",
    )
...
def student_user(db):
    user = UserProfile.objects.create_user(
        username="student_scores",
        password="testpass123",
        user_type="student",
        nick_name="Scores Student",
    )
...
def test_paper(db, teacher_user, subject, multiple_choice_question, true_false_question):
    ...
    TestPaperTestQ.objects.create(
        test_paper=paper, test_question=multiple_choice_question, score=10, order=1
    )
    TestPaperTestQ.objects.create(
        test_paper=paper, test_question=true_false_question, score=5, order=2
    )
    return paper
...
@pytest.fixture
def submitted_score(
    db, examination, student_user, test_paper, multiple_choice_question, true_false_question
):
    """제출된 성적"""
    return TestScores.objects.create(
        ...
        time_used=60,
        detail_records={
            str(multiple_choice_question.id): {
                "answer": "2",
                "is_correct": True,
                "score": 10,
                "max_score": 10,
            },
            str(true_false_question.id): {
                "answer": "1",
                "is_correct": True,
                "score": 5,
                "max_score": 5,
...
class TestExamStatistics:
    ...
    def test_exam_statistics(
        self, api_client, teacher_user, examination, student_user, student_user2, submitted_score
    ):
        """성적 통계 조회"""
        # 학생들 등록
    ...
    def test_exam_statistics_no_submissions(
        self, api_client, teacher_user, examination, student_user
    ):
        """제출 없는 경우 통계"""
        ExamStudentsInfo.objects.create(exam=examination, student=student_user.studentsinfo)
...
class TestStudentScoreDetail:
    ...
    def test_student_score_detail(
        self, api_client, teacher_user, examination, student_user, submitted_score
    ):
        """개별 학생 성적 상세 조회"""
        api_client.force_authenticate(user=teacher_user)
        response = api_client.get(
            f"/api/v1/scores/exam/{examination.id}/student/{student_user.studentsinfo.id}/"
        )
    ...
    def test_student_score_detail_not_creator(
        self, api_client, another_teacher, examination, student_user
    ):
        """시험 작성자가 아닌 교사는 조회 불가"""
        api_client.force_authenticate(user=another_teacher)
        response = api_client.get(
...
class TestManualGrade:
    ...
    def test_manual_grade_success(
        ...
        response = api_client.post(
            f"/api/v1/scores/{submitted_score.id}/grade/", data, format="json"
        )

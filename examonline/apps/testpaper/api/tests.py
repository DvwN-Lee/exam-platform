...
class TestPaperQuestionManagement:
    ...
    def test_create_paper_with_questions(
        self, api_client, teacher_user, subject, question1, question2
    ):
    ...
    def test_remove_question_from_paper(self, api_client, teacher_user, test_paper, question1):
        ...
        api_client.force_authenticate(user=teacher_user)
        url = reverse(
            "testpaper-remove-question", kwargs={"pk": test_paper.id, "question_id": question1.id}
        )
        response = api_client.delete(url)
        assert response.status_code == status.HTTP_204_NO_CONTENT
        test_paper.refresh_from_db()
...
class TestValidationErrors:
    ...
    def test_update_with_duplicate_questions_fails(
        self, api_client, teacher_user, test_paper, subject
    ):
        """업데이트 시 중복 문제 추가 실패"""

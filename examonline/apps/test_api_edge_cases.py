...
class TestAuthenticationEdgeCases:
    ...
    def setup(self, db):
        ...
        self.user = UserProfile.objects.create_user(
            username="testuser", password="testpass123", user_type="teacher"
        )
    ...
    def test_lowercase_bearer(self):
        ...
        response = self.client.post(
            "/api/v1/auth/token/", {"username": "testuser", "password": "testpass123"}
        )
...
class TestPaginationEdgeCases:
    ...
    def setup(self, db):
        ...
        # 로그인
        response = self.client.post(
            "/api/v1/auth/token/", {"username": "teacher", "password": "testpass"}
        )
...
class TestExamStateTransitions:
    ...
    def test_exam_cannot_modify_after_completion(self):
        ...
        response = self.client.patch(
            f"/api/v1/examinations/{exam.id}/", {"exam_name": "Updated Name"}
        )
...
class TestQuestionDeleteEdgeCases:
    ...
    def setup(self, db):
        ...
        self.other_teacher = UserProfile.objects.create_user(
            username="other_teacher", password="testpass", user_type="teacher"
        )
        self.subject = SubjectInfo.objects.create(subject_name="Test Subject")
...
class TestTestPaperValidation:
    ...
    def test_create_testpaper_passing_score_exceeds_total(self):
        ...
        data = {
            ...
            "questions": [{"question_id": self.question.id, "score": 10, "order": 1}],
        }
...
class TestExamTakingEdgeCases:
    ...
    def setup(self, db):
        ...
        self.student_user = UserProfile.objects.create_user(
            username="student", password="testpass", user_type="student"
        )
        self.student = StudentsInfo.objects.create(
            user=self.student_user, student_name="Test Student"
        )
        ...
        self.correct_option = OptionInfo.objects.create(
            test_question=self.question, option="Correct Answer", is_right=True
        )
        OptionInfo.objects.create(
            test_question=self.question, option="Wrong Answer", is_right=False
        )
    ...
    def test_submit_without_starting(self):
        ...
        new_student_user = UserProfile.objects.create_user(
            username="new_student", password="testpass", user_type="student"
        )
        new_student = StudentsInfo.objects.create(user=new_student_user, student_name="New Student")
        ExamStudentsInfo.objects.create(exam=self.exam, student=new_student)

        # 새 학생 로그인
        response = self.client.post(
            "/api/v1/auth/token/", {"username": "new_student", "password": "testpass"}
        )
        token = response.data["access"]
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
    ...
    def test_unenrolled_student_cannot_take_exam(self):
        ...
        unenrolled_user = UserProfile.objects.create_user(
            username="unenrolled", password="testpass", user_type="student"
        )
        StudentsInfo.objects.create(user=unenrolled_user, student_name="Unenrolled Student")

        # 로그인
        response = self.client.post(
            "/api/v1/auth/token/", {"username": "unenrolled", "password": "testpass"}
        )
        token = response.data["access"]
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        ...
        # 시험 시작 시도
        start_response = self.client.post(f"/api/v1/exams/{self.exam.id}/start/")
        assert start_response.status_code in [status.HTTP_403_FORBIDDEN, status.HTTP_404_NOT_FOUND]

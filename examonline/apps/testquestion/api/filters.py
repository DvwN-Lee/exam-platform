...
class QuestionFilter(django_filters.FilterSet):
    ...
    subject_name = django_filters.CharFilter(
        field_name="subject__subject_name", lookup_expr="icontains"
    )

    # 문제 유형 필터
    tq_type = django_filters.ChoiceFilter(
        choices=TestQuestionInfo._meta.get_field("tq_type").choices
    )

    # 난이도 필터
    tq_degree = django_filters.ChoiceFilter(
        choices=TestQuestionInfo._meta.get_field("tq_degree").choices
    )

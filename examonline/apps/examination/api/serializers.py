...
class ExaminationDetailSerializer(CreatUserSerializerMixin, serializers.ModelSerializer):
    ...
    def get_testpaper(self, obj):
        ...
        if exam_papers is None:
            ...
            exam_papers = list(
                ExamPaperInfo.objects.filter(exam=obj).select_related("paper", "paper__subject")
            )
...
class ExaminationUpdateSerializer(serializers.ModelSerializer):
    ...
    duration = serializers.IntegerField(
        write_only=True, required=False, help_text="시험 시간 (분 단위)"
    )
...
class EnrollStudentsSerializer(serializers.Serializer):
    ...
    def validate_student_ids(self, value):
        ...
        # 존재하는 학생인지 검증
        existing_students = StudentsInfo.objects.filter(id__in=unique_ids).values_list(
            "id", flat=True
        )
...
class AnswerItemSerializer(serializers.Serializer):
    ...
    answer = serializers.CharField(
        allow_blank=True, required=False, help_text="답안 (객관식: 옵션 ID, 주관식: 텍스트)"
    )

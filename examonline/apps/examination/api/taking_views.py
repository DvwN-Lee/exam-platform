...
from testpaper.models import TestPaperTestQ, TestScores
...
class ExamTakingViewSet(viewsets.ViewSet):
    ...
    def available_exams(self, request):
        ...
        available_exams = (
            ExaminationInfo.objects.filter(
                id__in=student_exams, start_time__lte=now, end_time__gte=now
            )
            .select_related("subject", "create_user")
            .prefetch_related("exampaperinfo_set__paper")
        )

        # 제출된 시험 ID Set 조회 (N+1 방지)
        submitted_exam_ids = set(
            TestScores.objects.filter(
                exam_id__in=student_exams, user=student_info, is_submitted=True
            ).values_list("exam_id", flat=True)
        )
        ...
        return Response(
            {"count": len(exams_data), "next": None, "previous": None, "results": exams_data},
    ...
    def exam_info(self, request, pk=None):
        ...
        # 문제 조회
        paper_questions = (
            TestPaperTestQ.objects.filter(test_paper=paper)
    ...
    def start(self, request, pk=None):
        ...
        # 시험 시작 기록 (select_for_update + try-except로 Race Condition 방지)
        with transaction.atomic():
            try:
                test_score = TestScores.objects.select_for_update().get(
                    exam=exam, user=student_info
                )
                if not test_score.start_time:
    ...
    def submit(self, request, pk=None):
        ...
        if not test_score or not test_score.start_time:
            return Response(
                {"detail": "시험을 시작하지 않았습니다."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # 1. 문제 정보 bulk 조회
        questions_dict = {q.id: q for q in TestQuestionInfo.objects.filter(id__in=question_ids)}

        # 2. 시험지-문제 매핑 정보 bulk 조회
        paper_questions_dict = {
            pq.test_question_id: pq
            for pq in TestPaperTestQ.objects.filter(
                test_paper=test_score.test_paper, test_question_id__in=question_ids
            )
        }
        ...
        correct_options_dict = {
            ...
            for opt in OptionInfo.objects.filter(test_question_id__in=question_ids, is_right=True)
        }
        ...
        logger.info(
            f"[{submit_type}] Saved TestScores ID: {test_score.id}, is_submitted: {test_score.is_submitted}, exam: {exam.id}, user: {student_info.id}"
        )
    ...
    def my_submissions(self, request):
        ...
        # 제출된 시험만 조회
        submissions = (
            TestScores.objects.filter(user=student_info, is_submitted=True)
            .select_related("exam__subject", "exam__create_user", "test_paper")
        ...
        if all_question_ids:
            questions_dict = {
                q.id: q
                for q in TestQuestionInfo.objects.filter(id__in=all_question_ids)
    ...
    def result(self, request, pk=None):
        ...
        test_score = (
            TestScores.objects.select_related("test_paper__subject")
            .filter(exam=exam, user=student_info)
            .first()
        )
        logger.info(
            f"[RESULT] Found test_score: {test_score.id if test_score else None}, is_submitted: {test_score.is_submitted if test_score else None}"
        )
        ...
        if question_ids:
            questions_dict = {
                q.id: q
                for q in TestQuestionInfo.objects.filter(id__in=question_ids)
                .select_related("subject")
                .prefetch_related("optioninfo_set")
            }

...
def teacher_user(db):
    return UserProfile.objects.create_user(
        username="teacher_taking",
        password="testpass123",
        user_type="teacher",
        nick_name="Taking Teacher",
    )
...
class TestExamSubmit:
    ...
    def test_submit_answers_correct(
        ...
        # 정답 ID 찾기
        correct_mc_option = OptionInfo.objects.get(
            test_question=multiple_choice_question, is_right=True
        )
        ...
        response = api_client.post(
            f"/api/v1/exams/{ongoing_examination.id}/submit/", data, format="json"
        )
    ...
    def test_submit_answers_incorrect(
        ...
        # 오답 ID 찾기
        wrong_mc_option = OptionInfo.objects.filter(
            test_question=multiple_choice_question, is_right=False
        ).first()
        wrong_tf_option = OptionInfo.objects.filter(
            test_question=true_false_question, is_right=False
        ).first()
            ...
            "answers": [
                {
                    ...
                    "answer": str(correct_mc_option.id),
                },  # 정답
...
class TestSaveDraft:
    ...
    def test_save_draft_success(self, api_client, student_user, ongoing_examination):
        ...
        response = api_client.post(
            f"/api/v1/exams/{ongoing_examination.id}/save-draft/", data, format="json"
        )

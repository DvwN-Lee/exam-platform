    ...
    def __call__(self, value):
        if value.size > self.max_size_bytes:
            raise ValidationError(
                self.message,
                code=self.code,
                params={"max_size": self.max_size_mb, "actual_size": value.size / (1024 * 1024)},
            )

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.max_size_bytes == other.max_size_bytes
...
class ImageMimeTypeValidator:
    ...
    def __eq__(self, other):
        return isinstance(other, self.__class__) and set(self.allowed_types) == set(
            getattr(other, "allowed_types", [])
        )
